...
namespace SGF.Network.KCP
{
    public class KCPSocket
    {
        ...
        private void OnEditorPlayModeChanged()
        {
            ...
            {
                UnityEngine.Debug.Log("OnEditorPlayModeChanged()");
                UnityEditor.EditorApplication.playmodeStateChanged -= OnEditorPlayModeChanged;
                Dispose();
            }
        }
        ...
        public void Dispose()
        {
            ...
            if (m_SystemSocket != null)
            {
                ...
                {
                    UnityEngine.Debug.Log("Close() " + e.Message + e.StackTrace);
                }
        ...
        public bool SendTo(IMessage message, IPEndPoint remotePoint)
        {
            KCPProxy proxy = GetKcp(remotePoint);
            if (proxy != null)
            {
                return proxy.DoSend(message);
            }
            return false;
        }
        ...
        private void Thread_Recv()
        {
            ...
            UnityEngine.Debug.Log("Thread_Recv() Begin ......");
            while (m_IsRunning)
            {
                try
                {
                ...
                {
                    UnityEngine.Debug.Log("Thread_Recv() " + e.Message + "\n" + e.StackTrace);
                    Thread.Sleep(10);
                }
            }
            UnityEngine.Debug.Log("Thread_Recv() End!");
        }
